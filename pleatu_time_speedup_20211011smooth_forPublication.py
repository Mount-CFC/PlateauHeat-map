...
#%%    
def gauss_fit(start_index, end_index, traces_per_unit, points=5):
    fit_max = []
    fit_avg = []
    for i in range(start_index, end_index):
        miu = np.mean(pleatu[i*traces_per_unit : (i+1)*traces_per_unit])
        temp = bins[0:bins_set]
        maxima = np.mean(temp[value[i] == np.max( value[i] )] )
    ...
    ax[1].plot(x_max, y, color = 'r', linewidth = 4, marker = 'D', markersize = 10)
    ax[0].set_xlabel('Arvage length(nm)',fontsize=18)
    ax[1].set_xlabel('Max length(nm)',fontsize=18)
    ...
    # https://zhuanlan.zhihu.com/p/105053717
    x_smooth = index_y[start_index : end_index]
    y1 = x_max
    y0 = x_avg
    
    s1 = UnivariateSpline(x_smooth,y1, s=0.5) #平滑参数s
    s0 = UnivariateSpline(x_smooth,y0, s=1)
    xs = np.linspace(index_y[start_index],index_y[end_index-1],points)#最后一个值是连成曲线的点数
    
    ys1 = s1(xs)
    ys0 = s0(xs)
    ax[1].plot(ys1,xs,color = 'black',linewidth = 3)
    ax[0].plot(ys0,xs,color = 'black',linewidth = 3)
...
if __name__ == '__main__':
    filepath = r'Z:/Data/lutaige/data_processing/20211021_0p05V_menshutkin/20220304redo_1-150min_remove81-90.txt'
    #复制黏贴地址可能出错，删去前面的字符就可以了
    ...
    melted_df = pivot_dataframe.melt(id_vars = 'time',
                              value_vars = bins[0:bins_set]) #第一个数字可以调整图片的开始坐标
    ...
    # https://blog.csdn.net/qq_40130759/article/details/79687288
    target = target.sort_index(ascending=False)
    ...
    # https://www.pythonheidong.com/blog/article/710693/341fdd83ed921e3d1c1c/
    #设置colorbar的刻度字体大小
    cax = plt.gcf().axes[-1]
    ...
    # https://blog.csdn.net/calmdownhhz/article/details/111555876
    ax.set_xlabel('Length distribution / nm',fontsize=30, fontproperties="Arial" ,labelpad=0)
    ax.set_ylabel('Time / min',fontsize=30, fontproperties="Arial" ,labelpad=-4)
    plt.rcParams['font.sans-serif'] = 'SimHei'#设置中文显示
    plt.rcParams['axes.unicode_minus'] = False #更改中文字体后，会导致坐标轴中的部分字符无法显示，同时要更改axes.unicode_minus参数
    plt.tick_params(labelsize=17)
    
    # 设置刻度间隔
    ax.set_xticks(np.arange(bins_set//10)*10)
    ax.set_xticklabels(bins[0:bins_set:10],  rotation=0) 
    
    
    # y轴共 end_time//time_bin 个刻度格,每 2 个一个格子
    ax.set_yticks(np.arange(0, end_time//time_bin, 2)+0.5)
    ax.set_yticklabels(np.arange(1,14)[::-1]*10, rotation=0)
    ...
    start_time = 0 #画图时开始的时间坐标,默认为0
    # start_time = 30
    # end_time = mintues   #画图时结束的时间坐标，默认到结束
    end_time = 125'''
    
    # https://blog.csdn.net/u010358304/article/details/78906768?spm=1001.2101.3001.6650.2&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-2.essearch_pc_relevant&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-2.essearch_pc_relevant
    labels = ax.get_xticklabels() + ax.get_yticklabels()
    [label.set_fontname('Arial') for label in labels]
    
    # https://blog.csdn.net/sinat_23133783/article/details/109527366  加边框
    ...
    # xmajorLocator   = MultipleLocator(0.06) #将x主刻度标签设置为20的倍数
    # xmajorFormatter = FormatStrFormatter('%5.1f') #设置x轴标签文本的格式
    # xminorLocator   = MultipleLocator(0.06) #将x轴次刻度标签设置为5的倍数
  ...
  #%%
    gauss_fit(start_index, end_index, traces_per_unit,points = 100)#拟合曲线的条数
